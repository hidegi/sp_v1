...
namespace sp
{
    ...
    void Renderer::debugPrint()
    {
        printf("=============================DEBUG PRINT:=============================\n");

        for(auto& m : m_drawables)
        {
            int handle = m.states.texture ? m.states.texture->getHandleGL() : 0;
            printf("drawable %lld, vertex entry: %lld, index entry: %lld, first index: %lld, vertices:\n", m.id, m.vertex_entry, m.index_entry, m.first_index);
            Drawable* drawable = m.drawable.lock()->client;
            for(size_t i = 0; i < drawable->m_vertices.size(); i++)
            {
                const sp::Vertex& vertex = drawable->m_vertices[i];
                printf("(v %lld): %f %f, color: %u %u %u %u, tex coords: %f %f\n", i, vertex.position.x, vertex.position.y, vertex.color.r, vertex.color.g,
                       vertex.color.b, vertex.color.a, vertex.texCoords.x, vertex.texCoords.y);
            }
            /*
            printf("\ncompare equality (positions): \n");
            for(size_t i = m.vertex_entry; i < i + m.vertex_count; i++)
            {
                const sp::Vertex& vertex = m_positions[i];
                printf("(v %lld): %f %f, ", i, vertex.position.x, vertex.position.y);
            }
            */

            printf("indices: \n");
            for(size_t i = 0; i < drawable->m_indices.size(); i++)
            {
                unsigned int index = drawable->m_indices[i];
                printf("%d ", index + m.first_index);
            }

            /*
            printf("\ncompare equality (indices): \n");
            for(size_t i = m.index_entry; i < i + m.index_count; i++)
            {
                unsigned int index = m_indices[i];
                printf("%d ", m.first_index);
            }
        ...
        printf("vertex count: %lld, actual vertex count: %lld (vertices) = %lld (colors) = %lld (tex coords)\n", m_vertex_count, m_positions.size(), m_colors.size(), m_tex_coords.size());
        printf("index count: %lld, actual index count: %lld\n", m_index_count, m_indices.size());

        printf("\n");
        for(size_t i = 0; i < m_positions.size(); i++)
        {
            const sp::vec2f& vertex = m_positions[i];
            const sp::Color& color = m_colors[i];
            const sp::vec2f& tc = m_tex_coords[i];
            printf("(v %lld): %f %f, color: %u %u %u %u, tex coords: %f %f\n", i, vertex.x, vertex.y, color.r, color.g, color.b, color.a, tc.x, tc.y);
        }
        printf("\n");
    ...
    void Renderer::ensureResize()
    {
        ...
        */
        ...
        for(auto& meta : m_drawables)
        {
            ...
            {
                m_positions.push_back(vertices[i].position);
                m_colors.push_back(vertices[i].color);
                m_tex_coords.push_back(vertices[i].texCoords);
            }
            */
            std::vector<Vertex>& vertices = ptr->client->m_vertices;
            if(meta.vertex_count != ptr->vertex_count ) //ptr->drawable->m_vertices.size()
            {
                ...
                printf("vertex count (resize): %lld\n", m_vertex_count);
                ...
                m_tex_coords.resize(m_vertex_count);
                ptr->update = false;
                //meta.vertex_count = vertices.size();
            }

            for(size_t i = 0; i < meta.vertex_count; i++)
            {
                m_positions  [i + meta.vertex_entry] = vertices[i + ptr->vertex_entry].position + ptr->position;
                m_colors     [i + meta.vertex_entry] = vertices[i + ptr->vertex_entry].color;
                m_tex_coords [i + meta.vertex_entry] = vertices[i + ptr->vertex_entry].texCoords;
            }
            counter += vertices.size();
            previous = &meta;
        }

        printf("ENSURE RESIZE: (actual count) %lld, (virtual count) %lld\n", counter, m_vertex_count);
    }
        ...
        for(auto& meta : m_drawables)
        {
            ...
            if(meta.vertex_count != ptr->vertex_count ) //ptr->drawable->m_vertices.size()
            {
                ...
                else //new_count < old_count..
                {
                    ...
                    printf("lessen case..\n");
                    printf("entry: %lld, count: %lld, size: %lld", meta.vertex_entry, new_count, m_positions.size());
                    m_positions.erase(m_positions.begin() + meta.vertex_entry + new_count, m_positions.begin() + meta.vertex_entry + old_count);
                    m_colors.erase(m_colors.begin() + meta.vertex_entry + new_count, m_colors.begin() + meta.vertex_entry + old_count);
                    m_tex_coords.erase(m_tex_coords.begin() + meta.vertex_entry + new_count, m_tex_coords.begin() + meta.vertex_entry + old_count);
                }
            ...
            */
            ...
            if(ptr->update)
            {
                ...
                for(size_t i = 0; i < length; i++)
                {
                    ...
                    //printf("ok 2.. vertex count: %lld, entry: %lld, client size: %lld\n", length, entry, vertices.size());
